...
new class extends Component {
    ...
    {
        $cards = Card::latest()->get()->take(50)->sortBy('-created_at');
        return view('components.⚡cards-table', ['cards' => $cards]);
    }
};
?>
            ...
            <tr>
                <td class="py-2 px-4 border-b border-gray-200">
                    <a class="hover:underline"
                       href="{{ route('cards.player', $card->player) }}">{{ $card->player->first_name }} {{ $card->player->last_name }}</a>
                </td>
                <td class="py-2 px-4 border-b border-gray-200">
                    <a class="hover:underline"
                       href="{{ route('cards.cardset', $card->cardSet) }}">{{ $card->cardSet->year }} {{ $card->cardSet->card_set_name }}</a>
                </td>
